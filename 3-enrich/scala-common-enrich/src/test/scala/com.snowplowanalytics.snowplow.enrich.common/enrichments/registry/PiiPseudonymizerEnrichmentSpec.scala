...
class PiiPseudonymizerEnrichmentSpec extends Specification with ValidationMatchers {
  def is = s2"""
    ...
    Hashing configured fields in JSON should work                                                           $e2
  ...
  def e1 = {
    val enrichmentMap = Map(
      ("ip_lookups" -> IpLookupsEnrichment(Some(("geo", new URI("/ignored-in-local-mode/"), "GeoIPCity.dat")),
                                           Some(("isp", new URI("/ignored-in-local-mode/"), "GeoIPISP.dat")),
                                           None,
                                           None,
                                           None,
                                           true)),
      ("pii_pseudonymizer_config" -> PiiPseudonymizerEnrichment(
        List(
